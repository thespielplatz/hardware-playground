...
void RequestBytes_(uint num_bytes) {
  Wire.requestFrom((uint8_t)SI47xx_I2C_ADDR, (uint8_t)num_bytes);
  while (Wire.available() < num_bytes)
    yield();
}

void Si47xx::SendCommand_(uint len) {
  Wire.beginTransmission((uint8_t)SI47xx_I2C_ADDR);
...
bool Si47xx::Start(uint I2C_SDA_PIN, uint I2C_SCL_PIN, uint RADIO_RESET_PIN, bool use_i2s_input) {
